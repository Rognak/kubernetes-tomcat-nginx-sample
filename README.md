...
$ vagrant up
```
Данная команда создаст 4 виртуальные машины в режиме "Сетевой мост". Во время разворачивания VirtualBox будет запрашивать, через какое устройство обеспечить выход в сеть - необходимо выбрать для каждой из машин параметр wlan (wlp3s0).
...
Kubernetes не поддерживает стандартные (облачные) балансировщики нагрузки (Load Balancer) для "bare metal" кластеров (наш случай), поэтому в качестве него мы будем использовать nginx-ingress.  
...
https://192.168.0.12:30470/ (Обратите внимание на порт: вы должны подставить своё значение, на которое перенаправляется порт 443 сервиса ingress-nginx см. "Настройка nginx-ingress" )

В случае успеха вы должны увидеть сообщение приветствия от tomcat.
